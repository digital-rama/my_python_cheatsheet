...
and 'timedelta' classes. All are immutable and hashable.
...
' = resolve_imaginary()' fixes DTs that fall into the missing hour.
Now
...
<dict> = vars(<object>)                    # Dict of writable attributes. Also .__dict__.
<bool> = hasattr(<object>, '')  # Checks if getattr() raises an AttributeError.
value  = getattr(<object>, '')  # Raises AttributeError if attribute is missing.
setattr(<object>, '', value)    # Only works on objects with __dict__ attribute.
delattr(<object>, '')           # Equivalent to `del .`.
...
<class> = type('', <parents_tuple>, <attributes_dict>)
...
<Image> = Image.new('', (width, height))  # Also: `color=`.
<Image> = Image.open(<path>)                    # Identifies format based on file contents.
<Image> = <Image>.convert('')             # Converts image to the new mode.
...
<Wave_read>  = wave.open('', 'rb')        # Opens the WAV file.
...
<Wave_write> = wave.open('', 'wb')        # Truncates existing file.
...
samples_b   = b''.join(struct.pack(', int(f * 30000)) for f in samples_f)
...
<Surf> = pg.image.load('')                # Loads the image. Format depends on source.
<Surf> = <Surf>.subsurface(<Rect>)              # Returns a subsurface.



<Surf>.fill(color)                              # Tuple, Color('#rrggbb[aa]') or Color().
...
arc(<Surf>, color, <Rect>, from_rad, to_rad)    # Also: ellipse(, color, )
rect(<Surf>, color, <Rect>)                     # Also: polygon(, color, points)
...
<Font> = pg.font.SysFont('', size)        # Loads the system font or default if missing.
<Font> = pg.font.Font('', size)           # Loads the TTF file. Pass None for default.
...
<Sound> = pg.mixer.Sound('')              # Loads the WAV file.
...
<el> = <Sr>.loc[key]                          # Or: .iloc[index]
<Sr> = <Sr>.loc[keys]                         # Or: .iloc[indexes]
<Sr> = <Sr>.loc[from_key : to_key_inclusive]  # Or: .iloc[from_i : to_i_exclusive]



<el> = <Sr>[key/index]                        # Or: .key
<Sr> = <Sr>[keys/indexes]                     # Or: []
<Sr> = <Sr>[bools]                            # Or: .i/loc[bools]
...
<el> = <Sr>.sum/max/mean/idxmax/all()         # Or: .aggregate()
<Sr> = <Sr>.rank/diff/cumsum/ffill/interpl()  # Or: .agg/transform()
<Sr> = <Sr>.fillna(<el>)                      # Or: .apply/agg/transform/map()
...
Last result has a hierarchical index. Use '[key_1, key_2]' to get its values.
...
<el>    = <DF>.loc[row_key, column_key]       # Or: .iloc[row_index, column_index]
<Sr/DF> = <DF>.loc[row_key/s]                 # Or: .iloc[row_index/es]
<Sr/DF> = <DF>.loc[:, column_key/s]           # Or: .iloc[:, column_index/es]
<DF>    = <DF>.loc[row_bools, column_bools]   # Or: .iloc[row_bools, column_bools]



<Sr/DF> = <DF>[column_key/s]                  # Or: .column_key
...
<DF>    = <DF>.filter('', axis=1)      # Only keeps columns whose key matches the regex.
...
<Sr> = <DF>.sum/max/mean/idxmax/all()         # Or: .apply/agg/transform()
<DF> = <DF>.rank/diff/cumsum/ffill/interpl()  # Or: .apply/agg/transform()
<DF> = <DF>.fillna(<el>)                      # Or: .applymap()
...
<DF> = <GB>.sum/max/mean/idxmax/all()         # Or: .apply/agg()
<DF> = <GB>.rank/diff/cumsum/ffill()          # Or: .aggregate()  
<DF> = <GB>.fillna(<el>)                      # Or: .transform()
...
<R_Sr/R_DF>      = <R_DF/R_GB>[column_key/s]        # Or: .column_key
<Sr/DF/DF>       = <R_Sr/R_DF/R_GB>.sum/max/mean()  # Or: .apply/agg()
...
<Figure> = line(<DF>, x=<col_name>, y=<col_name>)        # Or: line(x=, y=)
<Figure>.update_layout(margin=dict(t=0, r=0, b=0, l=0))  # Or: paper_bgcolor='rgba(0, 0, 0, 0)'
<Figure>.write_html/json/image('')                 # Also: 
.show()
